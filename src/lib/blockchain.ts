...
import StorageBandits from '../../public/abis/StorageBandits.json'
import StorageBanditsLocal from '../../abis/StorageBandits.json'
import { NetwokIds } from "../constants/networks";
import { StorageBanditsFile } from "../interfaces/StorageBandits-file.interface";
import { create } from 'ipfs-http-client'; //TO be deleted if caused errors

const projectId = process.env.NEXT_PUBLIC_INFURA_IPFS_PROJECT_ID // to
const projectSecret = process.env.NEXT_PUBLIC_INFURA_IPFS_PROJECT_SECRET //be 
const projectIdAndSecret = `${projectId}:${projectSecret}` //deleted
...
export class AppBlockchain {
    ...
    async getFilesMetadata(): Promise<StorageBanditsFile[]> {
        const methods = this.contract.methods;
        const filesCount = await methods.fileCount().call();
        const filesMetadata: StorageBanditsFile[] = [];
    ...
    private async initializeContractLocal() {
        ...
        const networkData = (StorageBanditsLocal as any).networks[networkId];
        if (networkData) {
            this.contract = new this.web3.eth.Contract(
                (StorageBanditsLocal as any).abi,
                networkData.address
            )
        }
    }
